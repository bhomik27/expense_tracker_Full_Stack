...
function printUserExpense(userExpense) {
    ...
    deleteButton.onclick = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:3000/expense/delete-expense/${userExpense._id}`, { headers: { "Authorization": token } });
    ...
    editButton.onclick = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:3000/expense/edit-expense/${userExpense._id}`, updatedUserExpenseData, { headers: { "Authorization": token } });
